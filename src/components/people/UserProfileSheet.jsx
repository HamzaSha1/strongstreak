...
// Props: person, currentUser, onClose
// NOTE: `following` prop is intentionally not accepted — this component owns
// its own follow state so mutations reflect instantly without prop-drilling.
export default function UserProfileSheet({ person, currentUser, onClose }) {
  ...
  // ── Follow state — fetched here so it stays fresh after mutations ─────────

  const { data: myFollowing = [] } = useQuery({
    queryKey: ['following', currentUser?.email],
    queryFn: () => base44.entities.Follow.filter({ follower_id: currentUser.email }),
    enabled: !!currentUser,
  });

  const isFollowing = myFollowing.some((f) => f.following_id === person.email);
  const isPrivate = person.is_private;
  const canViewContent = !isPrivate || isFollowing;

  // Pending follow request — only relevant for private accounts not yet followed
  ...
  // ── Content queries — gated by canViewContent ─────────────────────────────
  ...
  const followMutation = useMutation({
    mutationFn: async () => {
      if (!isPrivate) {
        // Public: direct follow / unfollow
        const existing = myFollowing.find((f) => f.following_id === person.email);
        if (existing) {
    ...
    onMutate: async () => {
      if (!isPrivate) {
        // Optimistically update following cache → instantly unlocks content for public accounts
        await queryClient.cancelQueries({ queryKey: ['following', currentUser?.email] });
        const prev = queryClient.getQueryData(['following', currentUser?.email]) || [];
        queryClient.setQueryData(
          ['following', currentUser?.email],
          isFollowing
            ? prev.filter((f) => f.following_id !== person.email)
            : [...prev, { follower_id: currentUser.email, following_id: person.email }]
        );
        return { prev };
      }

      // Private: optimistically update the request cache for instant button feedback
      const prevReqs = queryClient.getQueryData(['followRequest', currentUser?.email, person.email]) || [];
      if (pendingRequest) {
        queryClient.setQueryData(
          ['followRequest', currentUser?.email, person.email],
          prevReqs.filter((r) => r.id !== pendingRequest.id)
        );
      } else if (!isFollowing) {
        queryClient.setQueryData(
          ['followRequest', currentUser?.email, person.email],
          [...prevReqs, { requester_id: currentUser.email, target_id: person.email, status: 'pending' }]
        );
      }
      return { prevReqs };
    },
    onError: (_err, _vars, context) => {
      if (context?.prev) {
        queryClient.setQueryData(['following', currentUser?.email], context.prev);
      }
      if (context?.prevReqs) {
        queryClient.setQueryData(['followRequest', currentUser?.email, person.email], context.prevReqs);
      }
    },
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['myFollowRequests', currentUser?.email] });
    },
  });
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {isBlocked ? (
            <div className="flex flex-col items-center gap-3 py-16 px-8 text-center">
              ...
              <p className="text-sm text-muted-foreground">You have blocked this user. Tap the shield icon to unblock.</p>
            </div>
